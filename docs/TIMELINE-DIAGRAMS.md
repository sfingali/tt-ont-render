...
One vertical is one universe. A column is reserved for a single timeline for its whole length and is never reused, so the name at the head of the column is true everywhere below it. Columns are ordered depth-first in the order the author declares each split, which keeps a universe beside the one it separated from. Reuse previously kept the chart narrow, but placed several unrelated histories in one vertical, which cannot be named; the chart is wider now and scrolls horizontally.

Every column carries its universe's name at the head of the chart, with an origin tag beneath: **Already running**, **Begins at a split**, or **Origin not established**. The tag states the universe's origin and is independent of whether a prior-history line is drawn: a pre-existing universe present from the first row gets the tag but no line, because there is no charted history above it to be distinguished from.

A terminal outcome is drawn beside its fork with a short end bar, not as a tall, empty world column. The bar ends the selected perspective; it does not claim that the whole universe ceases to exist. Character travel keeps its blue dashed arrow and cannot create a destination implicitly.
