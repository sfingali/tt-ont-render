...
/** The kinds of source ids a profile can declare as drawn. */
export type DrawnKind = 'world' | 'agent' | 'event' | 'edge' | 'intervention' | 'outcome';

/**
 * One declaration from a profile: "this concrete ontology source id is carried
 * by a mark in the document I just returned." Profiles append these at the draw
 * site, so the declaration is generated by the same loops that emit the marks.
 */
export interface DrawnSource {
  kind: DrawnKind;
  id: string;
  /** short account of the mark that carries this source id */
  reason: string;
}

/**
 * Append a draw declaration once, preserving first-draw order. Pure and
 * deterministic; this is the only sanctioned way for a profile to add to
 * `RenderDoc.drawn`.
 */
export function declareDrawn(list: DrawnSource[], kind: DrawnKind, id: string, reason: string): void {
  if (!id) return;
  if (!list.some(d => d.kind === kind && d.id === id)) list.push({ kind, id, reason });
}
...
export interface RenderDoc {
  ...
  profileId: string;
  /**
   * Every ontology source id the profile actually drew. Coverage reconciles
   * this against the resolved Facts; a declaration naming an id the story does
   * not declare is a hard error, so invented coverage cannot be printed.
   */
  drawn: DrawnSource[];
}
